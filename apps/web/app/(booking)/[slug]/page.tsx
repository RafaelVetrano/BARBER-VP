...
import { BarbershopPage } from '@/components/booking/public-page/barbershop-page';
import { fetchBarbershop } from '@/lib/booking/server-api';
import { BOOKING_ORIGIN } from '@/lib/urls';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // Canonical ABSOLUTO quando o host do booking é conhecido (fase 11): no
  // frontend único a mesma rota responde em qualquer host, e sem isto a página
  // da barbearia poderia ser indexada também sob o domínio de marketing. Sem a
  // variável definida (dev), volta a ser relativa ao host atual.
  const canonical = BOOKING_ORIGIN ? `${BOOKING_ORIGIN}/${shop.slug}` : `/${shop.slug}`;
