# Agente — Auditoria e correção 1:1 da tela Serviços & Produtos

Projeto: BarberVP. Esta sessão corrige **apenas** a tela Serviços & Produtos (`/app/servicos-produtos`).
...
3. **`BARBER VP/Dashboard.dc.html`, linhas 1723–2022** — esta é a fonte de
...
4. A implementação atual:
   - `apps/web/app/(dashboard)/app/servicos-produtos/page.tsx` (151 linhas)
   - `apps/web/components/dashboard/catalog/` (2 arquivos)

## Inventário a conferir contra o `.dc.html`

Três abas no protótipo:
- **Serviços** — nome, duração, preço, comissão específica, cor, barbeiros que
  atendem, toggle ativo, menu ⋯.
- **Produtos** — estoque com `estoqueMin` e alerta de estoque baixo, custo,
  preço de venda, margem.
- **Calculadora de preço** — "Custos e parâmetros" + "Simulação", com o card
  "Disponível no plano Avançado" quando bloqueada.
...
- `GET|POST|PATCH|DELETE` do catálogo (`catalog-admin`)
- `POST /settings/price-calculator` (atrás de `calculadoraPreco`, Avançado)
...
- O Dashboard navega para cá pelo aviso de estoque mínimo do sino.
