# Agente — Auditoria e correção 1:1 da tela Equipe

Projeto: BarberVP. Esta sessão corrige **apenas** a tela Equipe (`/app/equipe`).
...
3. **`BARBER VP/Dashboard.dc.html`, linhas 2023–2239** — esta é a fonte de
...
4. A implementação atual:
   - `apps/web/app/(dashboard)/app/equipe/page.tsx` (208 linhas)
   - `apps/web/components/dashboard/team/` (3 arquivos)

## Inventário a conferir contra o `.dc.html`

Três views no protótipo (`isEquipeViewGrid` / `Escala` / `Convites`):
- **Grade** — cards de barbeiro com foto, especialidade, nota.
- **Escala** — horário por dia da semana COM intervalo de almoço
  (`lunchStart`/`lunchEnd`) e marcação de folga/férias.
- **Convites** — pendentes/aceitos, com o gate de `maxBarbeiros` do plano
  (403 `PLAN_LIMIT_REACHED`, já implementado em `PlanLimitsService`).
...
- `GET|POST|PATCH` de barbeiros (`team`)
- convites (`StaffInvite`)
- `WorkSchedule` / `ScheduleException`
...
- O limite de barbeiros por plano é server-side e já devolve 403 com
  mensagem própria — o front deve mostrar upsell, não engolir o erro.
