# Agente — Auditoria e correção 1:1 da tela Comissões

Projeto: BarberVP. Esta sessão corrige **apenas** a tela Comissões (`/app/comissoes`).
...
3. **`BARBER VP/Dashboard.dc.html`, linhas 1089–1228** — esta é a fonte de
...
4. A implementação atual:
   - `apps/web/app/(dashboard)/app/comissoes/page.tsx` (189 linhas)
   - `apps/web/components/dashboard/commissions/rule-modal.tsx`

## Inventário a conferir contra o `.dc.html`

- Regras `FIXED` (% único) e `TIERED` (faixas por faturamento — ex. até
  R$5.000 → 40%, até R$8.000 → 45%, acima → 50%).
- Extrato do período por barbeiro, com vales descontados.
- Fechamento do período (recalcula a faixa definitiva e trava).
- Card "Disponível no plano Profissional" quando `comissoes` está fora do plano
  — o controller INTEIRO é gated, então até o `BARBER` pedindo o próprio
  extrato toma 403 no Essencial.
...
- `GET|POST|PATCH /commissions/rules(/:id)`
- `GET /commissions/period?month=YYYY-MM`
- `POST /commissions/period/close`
- `GET|POST /commissions/vales`
...
- `BARBER` só vê a si mesmo — conferir que a tela respeita isso e não
  oferece seletor de barbeiro.
