# Agente — Auditoria e correção 1:1 da tela WhatsApp

Projeto: BarberVP. Esta sessão corrige **apenas** a tela WhatsApp (`/app/whatsapp`).
...
3. **`BARBER VP/Dashboard.dc.html`, linhas 1624–1722** — esta é a fonte de
...
4. A implementação atual:
   - `apps/web/app/(dashboard)/app/whatsapp/page.tsx` (117 linhas)

## Inventário a conferir contra o `.dc.html`

Blocos identificados no protótipo:
- **Conexão com WhatsApp** — estado "🟢 Conectado como (54) 9 9918-8533".
  Não existe endpoint de conexão (o driver é mock, fase 09). Decidir: card de
  estado do adapter, ou bloco explicitamente marcado como "em breve".
- **Automações** — os 6 eventos com template parametrizado
  (`{nome}`/`{data}`/`{horario}`/`{servico}`/`{barbeiro}`/`{link_agendamento}`).
  Aniversário/reativação/avaliação exigem `whatsappCompleto`, checado POR
  EVENTO — ler a lista sempre funciona.
- **Histórico de envios** — `NotificationOutbox` do tenant. Não há endpoint de
  tenant para isso (só `/admin/outbox`, que é super admin). Criar.
...
- O Dashboard navega para cá com `?evento=REACTIVATION` e `?evento=BIRTHDAY`
  (botões da faixa de alertas). **Conferir se são lidos** — se não, os dois
  botões chegam numa lista sem o evento aberto.
